...
export const saveGameData = async () => {
  try {
    ...
    const gameData = {
      story: "너를 이용해서 챗봇 게임을 만들려고 해. 나는 게임 플레이어고 너는 게임의 안내자 역할을 해줘. 게임의 적은 임의로 생성하지 말고 '적 목록'에 있는 적들로만 생성해줘. 유저는 공격할 캐릭터의 순서를 선택할 수 있다.",
      gameMechanics: {
        gameplay: "유저는 청룡, 백호, 주작, 현무 네 명의 캐릭터를 이용해서 하나의 적에게 공격할 수 있다. 유저는 공격할 캐릭터의 순서를 선택할 수 있다. 각각의 캐릭터에게 '공격', '필살기' 등의 명령어로 지시할 수 있다. 캐릭터, '공격' 또는 공격이 들어간 채팅을 입력하면 해당 캐릭터가 적에게 공격한다. '필살기' 또는 필살기가 들어간 채팅을 입력하면 해당 캐릭터가 적에게 특수공격을 발동한다. 적을 공격하면 적이 입은 데미지를 표시한다. 4명의 캐릭터가 턴을 마치면 적이 4명의 캐릭터에게 동시에 공격한다. 같은 캐릭터를 연속으로 선택하지 못하게 한다. 같은 캐릭터를 연속으로 선택할 경우 아직 회복하지 못해서 능력을 사용하지 못한다고 언급한다.",
        characterStats: {
          health: 100,
          attack: 10,
          defense: 10
